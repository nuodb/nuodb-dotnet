...
namespace NuoDb.EntityFrameworkCore.NuoDb.Scaffolding.Internal
{
    ...
    public class NuoDbDatabaseModelFactory : DatabaseModelFactory
    {
        ...
        private void GetTables(DbConnection connection, DatabaseModel databaseModel, IEnumerable<string> tables)
        {
            ...
            //         .AppendLine("FROM \"NuoDb_master\"")
            //         .Append("WHERE \"type\" IN ('table', 'view') AND instr(\"name\", 'NuoDb_') <> 1 AND \"name\" NOT IN ('")
        ...
        private void GetColumns(DbConnection connection, DatabaseTable table)
        {
            ...
            //     {
            //         var db = NuoDbConnection.Handle;
            //         var rc = NuoDb3_table_column_metadata(
            //             db,
            //             connection.Database,
            //             table.Name,
            //             columnName,
            ...
            //             out var _,
            //             out autoIncrement);
            //         NuoDbException.ThrowExceptionForRC(rc, db);
            //     }
            //
            //     table.Columns.Add(
            //         new DatabaseColumn
        ...
        private void GetPrimaryKey(DbConnection connection, DatabaseTable table)
        {
            ...
            {
                Table = table,
                Name = name.StartsWith("NuoDb_", StringComparison.Ordinal) ? string.Empty : name
            };
        ...
        private void GetUniqueConstraints(DbConnection connection, DatabaseTable table)
        {
            ...
            while (reader1.Read())
            {
                ...
                {
                    Table = table,
                    Name = constraintName.StartsWith("NuoDb_", StringComparison.Ordinal) ? string.Empty : constraintName
                };
        ...
        private void GetIndexes(DbConnection connection, DatabaseTable table)
        {
            ...
            command1.CommandText = new StringBuilder()
                ...
                .AppendLine("FROM pragma_index_list(@table)")
                .AppendLine("WHERE \"origin\" = 'c' AND instr(\"name\", 'NuoDb_') <> 1")
                .AppendLine("ORDER BY \"seq\";")
                .ToString();
