...
{
    public class QueryLoggingNuoDbTest: IClassFixture<NorthwindQueryNuoDbFixture<NoopModelCustomizer>>
{
    public QueryLoggingNuoDbTest(NorthwindQueryNuoDbFixture<NoopModelCustomizer> fixture)
    {
        Fixture = fixture;
        Fixture.TestSqlLoggerFactory.Clear();
    }

    protected NorthwindQueryNuoDbFixture<NoopModelCustomizer> Fixture { get; }
    ...
    public virtual void Queryable_with_parameter_outputs_parameter_value_logging_warning()
    {
        ...
        Assert.NotNull(customers);
        Assert.Contains(
            CoreResources.LogSensitiveDataLoggingEnabled(new TestLogger<NuoDbLoggingDefinitions>()).GenerateMessage(),
    ...
    public virtual void Skip_without_order_by()
    {
        ...
        Assert.Equal(
            CoreResources.LogRowLimitingOperationWithoutOrderBy(new TestLogger<NuoDbLoggingDefinitions>()).GenerateMessage(),
            Fixture.TestSqlLoggerFactory.Log[1].Message);
    }
    ...
    public virtual void FirstOrDefault_without_filter_order_by()
    {
        ...
        Assert.Equal(
            CoreResources.LogFirstWithoutOrderByAndFilter(new TestLogger<NuoDbLoggingDefinitions>()).GenerateMessage(),
            Fixture.TestSqlLoggerFactory.Log[1].Message);
    }
    ...
    public virtual void Distinct_used_after_order_by()
    {
        ...
        Assert.Equal(
            CoreResources.LogDistinctAfterOrderByWithoutRowLimitingOperatorWarning(new TestLogger<NuoDbLoggingDefinitions>())
                .GenerateMessage(),
            Fixture.TestSqlLoggerFactory.Log[1].Message);
    }
    ...
    public void SelectExpression_does_not_use_an_old_logger()
    {
        ...
        using (var context = new NorthwindNuoDbContext(CreateOptions(loggerFactory1)))
        {
        ...
        using (var context = new NorthwindNuoDbContext(CreateOptions(loggerFactory2)))
        {
            var _ = context.Customers.ToList();
        }
