    ...
    public class TestFixture1
    {
        ...
        public void TestDataType(string sqlType, object value, object expected)
        {
            using (NuoDbConnection connection = new NuoDbConnection(connectionString))
            {
                ...
                if (val is DateOnly)
                    Assert.AreEqual(DateOnly.Parse(expected.ToString()), val, string.Format("Expected [{0}]; is [{1}]", expected, val));
                else if (val is TimeOnly)
                    Assert.AreEqual(TimeOnly.Parse(string.Format("{0:HH:mm:ss.FFF}", expected)), TimeOnly.Parse(string.Format("{0:HH:mm:ss.FFF}", val)), string.Format("Expected [{0:HH:mm:ss.FFFFFF}]; is [{1:HH:mm:ss.FFFFFF}]", expected, val));
                else if (val is DateTime)
                    Assert.AreEqual(DateTime.Parse(string.Format("{0:yyyy-MM-dd HH:mm:ss.FFFFFF}", expected)), DateTime.Parse(string.Format("{0:yyyy-MM-dd HH:mm:ss.FFFFFF}", val)), string.Format("Expected [{0:yyyy-MM-dd HH:mm:ss.FFFFFF}]; is [{1:yyyy-MM-dd HH:mm:ss.FFFFFF}]", expected, val));
                else if (val is TimeSpan)
                    Assert.AreEqual(TimeSpan.Parse(string.Format("{0:HH:mm:ss.FFFFFF}", expected)), TimeSpan.Parse(string.Format("{0:HH:mm:ss.FFFFFF}", val)), string.Format("Expected [{0:HH:mm:ss.FFFFFF}]; is [{1:HH:mm:ss.FFFFFF}]", expected, val));
                else if (expected is ICollection)
                    CollectionAssert.AreEqual((ICollection)expected, (ICollection)val);
                else if (val.GetType() == expected.GetType())
                    Assert.AreEqual(expected, val, string.Format("Expected {0}: [{1}]; is [{2}]", expected.GetType(), expected, val));
                else
        ...
        public void TestDataTypeTime()
        {
            ...
            TestDataType("time", now);
            TestDataType("time", new TimeOnly(10, 30, 22));
